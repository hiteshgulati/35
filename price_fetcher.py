...
def get_quote_from_quandl (quandl_code, fetch_date = datetime.today(), type = "Close"):
    ...
    try:
        quote = quandl.get(quandl_code, start_date=fetch, end_date=fetch).Close[0]
    except IndexError:
...
def run():
    slice = {"NSE_Ticker":"", "Remarks":"", "DOA": datetime(1,1,1), "DOA_Open":0,"DOA_Close":0, "Next_Day_Close": 0,
    ...
    for i, item in price_file.iterrows():
        ...
        # for j in range(1,item.Additional_Date_Count):
